# Class Templates Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a class template system that auto-fills character stats, spells, abilities, gear, and traits from the 20 4AD core rulebook classes when selected from a dropdown.

**Architecture:** Static TypeScript data file (`src/data/classTemplates.ts`) holds all 20 class definitions. A utility function (`src/utils/applyClassTemplate.ts`) resolves a template into a `Partial<Character>` by rolling dice for gold, computing L1 stats, and picking random traits. The free-text class inputs in `CharacterInputGroup` and `TavernCreatorModal` are replaced with `<select>` dropdowns.

**Tech Stack:** React 19, TypeScript 5.7, Zustand 5, Vitest, existing `rollDice()` from `src/utils/dice.ts`

---

### Task 1: Create ClassTemplate types and data file with all 20 classes

**Files:**
- Create: `src/data/classTemplates.ts`

**Step 1: Create the data file with types and all 20 class definitions**
...
```bash
git add src/data/classTemplates.ts
git commit -m "feat: add 20 class templates from 4AD core rulebook"
```

---

### Task 2: Write failing tests for applyClassTemplate

**Files:**
- Create: `src/utils/applyClassTemplate.test.ts`
...
Run: `npx vitest run src/utils/applyClassTemplate.test.ts`
Expected: FAIL — module `./applyClassTemplate` not found

---

### Task 3: Implement applyClassTemplate utility

**Files:**
- Create: `src/utils/applyClassTemplate.ts`
...
```bash
git add src/utils/applyClassTemplate.ts src/utils/applyClassTemplate.test.ts
git commit -m "feat: add applyClassTemplate utility with tests"
```

---

### Task 4: Update CharacterInputGroup with class dropdown

**Files:**
- Modify: `src/components/setup/CharacterInputGroup.tsx:41-46`

**Step 1: Replace the free-text class input with a select dropdown**

Replace the class `<input>` (lines 41-46) with:
...
And after the select, add a conditional free-text input for custom class:
...
The `__custom__` option sets class to empty string, allowing free-text entry. If the user types something not in CLASS_NAMES, the custom input appears.

**Step 2: Verify dev server works**

Run: `npm run dev`
Expected: App loads, class dropdown visible in setup screen, selecting a class auto-fills fields
...
```bash
git add src/components/setup/CharacterInputGroup.tsx
git commit -m "feat: replace class text input with template dropdown in setup"
```

---

### Task 5: Update TavernCreatorModal with class dropdown

**Files:**
- Modify: `src/components/modals/TavernCreatorModal.tsx:17,66`
...
**Step 2: Replace the class input (line 66)**
...
**Step 3: Update handleCreate to include template data for gear/spells/abilities/traits**

In the `handleCreate` function, after the `charClass` assignment, apply template data:
...
```bash
git add src/components/modals/TavernCreatorModal.tsx
git commit -m "feat: add class template dropdown to tavern creator"
```
...
2. Select "Wizard" from dropdown → verify HP=3/3, GP in 4-24 range, 6 spells populated, abilities populated, 1 trait
3. Select "Halfling" → verify HP=4/4, 2 traits, rations 4-9
4. Select "Custom..." → verify free-text input appears
5. Switch to Troupe mode → Create new tavern hero → verify dropdown works there too
