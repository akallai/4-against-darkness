...
describe('useEncounterStore', () => {
  ...
  it('toggles enemy status', () => {
    ...
    expect(useEncounterStore.getState().encounters[0]?.status[0]).toBe(true)
    expect(useEncounterStore.getState().encounters[0]?.status[1]).toBe(false)
  })
